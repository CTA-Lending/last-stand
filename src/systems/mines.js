...
export function updateMines(t, enemies, dt) {
  ...
  for (let i = t.mines.length - 1; i >= 0; i--) {
    const m = t.mines[i];
    const trig = enemies.find(e => e.alive && !e.reachedEnd && !e.flying && dist(m.x, m.y, e.x, e.y) <= 14);
    if (!trig) continue;
    for (const e of enemies) if (e.alive && dist(m.x, m.y, e.x, e.y) <= t.splash) {
      e.hp -= computeDamage(t.damage, t.attackType || 'earth', e.armorType); e.hitFlash = 0.12;
    }
    t.mines.splice(i, 1);
